...
const App: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || 'មានបញ្ហាក្នុងការភ្ជាប់ទៅកាន់ប្រព័ន្ធ។ សូមពិនិត្យមើល API Key ក្នុង Environment Variables របស់អ្នក។');
    ...
    <div className={`min-h-screen flex flex-col transition-all duration-700 ${activeBgClass}`}>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className="flex flex-col gap-4">
          ...
          {error && (
            <div className="bg-red-500 text-white p-4 rounded-2xl text-sm flex items-center gap-3 shadow-lg">
